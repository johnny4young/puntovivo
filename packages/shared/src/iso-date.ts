 ...
 *
 * Shared because the browser needs the SAME rule as the server. Three
 * places in this codebase have independently reimplemented it and each one
 * hardened only the date-only branch while leaving the timestamp branch on
 * bare `Date.parse`, so a form happily offered a lot the server then
 * rejected. One implementation removes the class.
 *
