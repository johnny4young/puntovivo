 ...
 * ). This server-side contract describes configuration +
 * connectivity tests so the admin UI can list registered scanners
 * and signal status.
 *
 * No default driver ships in  — the contract is exported so
 * can drop in a `usb-hid` driver without contract churn.
 *
