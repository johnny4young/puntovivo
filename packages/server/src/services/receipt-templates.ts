...
/**
 * Resolves the active default template for a kind, falling back to the
 * most recently updated active row if no row carries `is_default = 1`
 * (defensive — the invariant should ensure a default exists, but a
 * data-migration bug could produce a kind with rows but no default and
 * the caller would otherwise receive `null` and break).
 */
export function getDefaultReceiptTemplate(
  db: DatabaseInstance,
  tenantId: string,
  kind: ReceiptTemplateKind
): ReceiptTemplateRecord | null {
