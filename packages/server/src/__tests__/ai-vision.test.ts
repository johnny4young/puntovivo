...
describe('extractInvoiceFromImage', () => {
  ...
  it('persists a failed-call audit row and surfaces AI_VISION_PARSE_FAILED for schema errors', async () => {
    ...
    const secret = 'PRIVATE_INVOICE_IN_PARSE_ERROR';
    generateObjectMock.mockRejectedValue(new Error(`No object generated by the model. ${secret}`));

    const parseError = await expectErrorCode(
      extractInvoiceFromImage(
        { db: getDatabase(), tenantId, siteId: null, userId: null },
        { imageBase64: 'aGVsbG8=', mimeType: 'image/png' },
        () => buildStubProvider()
    ...
    expect(parseError.message).toBe('Invoice could not be parsed');
    expect(parseError.message).not.toContain(secret);
    expect((parseError.cause as ServerErrorWithCode).details).toBeUndefined();
  ...
  it('persists a failed-call audit row and surfaces AI_PROVIDER_ERROR for transport errors', async () => {
    ...
    await enableAI(tenantId);
    const secret = 'PRIVATE_INVOICE_IN_PROVIDER_ERROR';
    generateObjectMock.mockRejectedValue(new Error(`upstream 502 bad gateway ${secret}`));

    const providerError = await expectErrorCode(
      extractInvoiceFromImage(
        { db: getDatabase(), tenantId, siteId: null, userId: null },
        { imageBase64: 'aGVsbG8=', mimeType: 'image/png' },
        () => buildStubProvider()
    ...
    expect(providerError.message).toBe('Vision provider call failed');
    expect(providerError.message).not.toContain(secret);
    expect((providerError.cause as ServerErrorWithCode).details).toBeUndefined();
