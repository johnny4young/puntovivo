/**
 * ENG-169 — input boundary hygiene (audit-closure W5).
 *
...
describe('image-URL scheme hardening (ENG-169)', () => {
  ...
  it('rejects dangerous schemes on product imageUrl but keeps relative + data:image', () => {
    ...
    expect(() =>
      updateProductInput.parse({ ...base, imageUrl: 'javascript:alert(1)' })
    ).toThrow();
    // Refine-only (no .url()): a relative path the field already accepted
    // stays valid, and data:image is allowed.
    expect(
      updateProductInput.parse({ ...base, imageUrl: '/uploads/p-1.png' }).imageUrl
    ).toBe('/uploads/p-1.png');
    expect(
...
describe('contact-email normalisation (ENG-169)', () => {
  ...
  it('trims + lowercases customer email', () => {
    expect(updateCustomerInput.parse({ id: 'c-1', version: 0, email: DIRTY }).email).toBe(
      CLEAN
    );
  });
  it('trims + lowercases provider email', () => {
    expect(updateProviderInput.parse({ id: 'pr-1', version: 0, email: DIRTY }).email).toBe(
      CLEAN
    );
  });
...
describe('verbose+prod boot guard (ENG-169)', () => {
  ...
  it('refuses to boot with verbose logging in production without the override', async () => {
    ...
    ).rejects.toThrow(/verbose logging is enabled/i);
  });
