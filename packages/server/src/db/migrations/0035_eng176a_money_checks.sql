-- ENG-176a — money model CHECK invariants (Step-1 of ENG-176).
--
-- This migration adds non-negative CHECK constraints to monetary
-- columns in 17 tables so the storage layer defends against negative
-- totals where they have no semantic meaning. The companion
-- 0036_eng176a_rounding_precision.sql migration adds the two-decimal
-- precision invariant after the application roundMoney sweep. The
-- decision between integer minor units and real + CHECK invariants is
-- recorded in docs/architecture/0009-money-storage-and-validation.md.
--
-- Two trade-offs worth flagging at the top of this migration:
--
--   1. SQLite cannot add a CHECK to an existing column in place — the
--      table has to be recreated. Drizzle-kit emits the canonical
--      12-step pattern per table (create __new_table → copy rows →
--      drop old → rename new → recreate indexes). With 17 affected
--      tables the migration is large but mechanical.
--
--   2. fiscal_documents, fiscal_document_items, and payment_outbox
--      ARE present in the current 0035 snapshot, but their CREATE
--      TABLE history lives in hand-appended migrations
--      (`0005_fiscal_documents.sql` and later) that predate the
--      drizzle-kit snapshot chain. The kit cannot generate a clean
--      incremental __new_X recreation against a missing baseline
--      snapshot without a manual `drizzle-kit introspect` + snapshot
--      rebuild, which would balloon this slice. Their CHECK invariants
--      are deferred to ENG-176b, which recreates those tables anyway
--      to add currency_code + exchange_rate. The Zod schemas at the
--      tRPC boundary (`schemas/fiscal*.ts`, `schemas/payments.ts`)
--      already enforce positive amounts + two-decimal precision before
--      any write reaches the DB.
--
-- BEFORE the recreations, a defensive UPDATE rounds any historical
-- value with > 2 decimals to two decimals. Drift could appear from
-- pre-ENG-166 imports, raw SQL CLI sessions, or a future feature
-- bug. The 0035 checks only require non-negative values, but this
-- prelude keeps recreated data cent-normalized before the companion
-- 0036 precision pass.

UPDATE `customers` SET `credit_limit` = round(`credit_limit`, 2) WHERE round(`credit_limit`, 2) != `credit_limit`;--> statement-breakpoint
UPDATE `products` SET
  `price` = round(`price`, 2),
  `price2` = round(`price2`, 2),
  `price3` = round(`price3`, 2),
  `cost` = round(`cost`, 2),
  `margin_amount1` = round(`margin_amount1`, 2),
  `margin_amount2` = round(`margin_amount2`, 2),
  `margin_amount3` = round(`margin_amount3`, 2),
  `initial_cost` = round(`initial_cost`, 2)
WHERE 1=1;--> statement-breakpoint
UPDATE `sales` SET
...
UPDATE `sale_returns` SET `refund_amount` = round(`refund_amount`, 2) WHERE round(`refund_amount`, 2) != `refund_amount`;--> statement-breakpoint
...
UPDATE `denomination_templates` SET `opening_float` = round(`opening_float`, 2) WHERE round(`opening_float`, 2) != `opening_float`;--> statement-breakpoint
...
UPDATE `purchase_items` SET
  ...
  `total` = round(`total`, 2)
WHERE 1=1;--> statement-breakpoint
UPDATE `purchase_returns` SET `return_amount` = round(`return_amount`, 2) WHERE round(`return_amount`, 2) != `return_amount`;--> statement-breakpoint
UPDATE `orders` SET
  `subtotal` = round(`subtotal`, 2),
  `total` = round(`total`, 2)
WHERE 1=1;--> statement-breakpoint
UPDATE `order_items` SET
  ...
  `total` = round(`total`, 2)
WHERE 1=1;--> statement-breakpoint
UPDATE `initial_inventory` SET `cost` = round(`cost`, 2) WHERE round(`cost`, 2) != `cost`;--> statement-breakpoint
