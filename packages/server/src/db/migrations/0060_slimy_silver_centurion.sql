...
CREATE TABLE `__new_sales` (
	...
	`customer_id` text,
	`price_tier` integer DEFAULT 1 NOT NULL,
	`customer_name_snapshot` text,
	`site_name_snapshot` text,
	`cashier_name_snapshot` text,
	`receipt_identity_snapshot_version` integer,
	...
	`receipt_presentation_snapshot_version` integer,
	`receipt_template_id_snapshot` text,
	`receipt_template_kind_snapshot` text,
	`receipt_template_name_snapshot` text,
	`receipt_template_layout_snapshot` text,
	`receipt_logo_url_snapshot` text,
	`receipt_locale_snapshot` text,
	`table_id` text,
	...
	CONSTRAINT "chk_sales_cash_session_or_draft" CHECK("__new_sales"."cash_session_id" IS NOT NULL OR "__new_sales"."status" IN ('draft', 'cancelled'))
);
--> statement-breakpoint
INSERT INTO `__new_sales`("id", "tenant_id", "sale_number", "customer_id", "price_tier", "customer_name_snapshot", "site_name_snapshot", "cashier_name_snapshot", "receipt_identity_snapshot_version", "company_name_snapshot", "company_tax_id_snapshot", "company_address_snapshot", "company_phone_snapshot", "company_email_snapshot", "customer_tax_id_snapshot", "receipt_presentation_snapshot_version", "receipt_template_id_snapshot", "receipt_template_kind_snapshot", "receipt_template_name_snapshot", "receipt_template_layout_snapshot", "receipt_logo_url_snapshot", "receipt_locale_snapshot", "table_id", "subtotal", "tax_amount", "discount_amount", "total", "currency_code", "exchange_rate_at_sale", "settle_currency_code", "tip_amount", "tip_method", "service_charge_amount", "service_charge_rate", "payment_method", "payment_status", "return_state", "status", "cash_session_id", "notes", "created_by", "checkout_started_at", "checkout_completed_at", "suspended_at", "suspended_by", "suspended_label", "reprint_count", "last_reprinted_at", "last_reprinted_by", "sync_status", "sync_version", "created_at", "updated_at") SELECT "id", "tenant_id", "sale_number", "customer_id", "price_tier", "customer_name_snapshot", "site_name_snapshot", "cashier_name_snapshot", "receipt_identity_snapshot_version", "company_name_snapshot", "company_tax_id_snapshot", "company_address_snapshot", "company_phone_snapshot", "company_email_snapshot", "customer_tax_id_snapshot", "receipt_presentation_snapshot_version", "receipt_template_id_snapshot", "receipt_template_kind_snapshot", "receipt_template_name_snapshot", "receipt_template_layout_snapshot", "receipt_logo_url_snapshot", "receipt_locale_snapshot", "table_id", "subtotal", "tax_amount", "discount_amount", "total", "currency_code", "exchange_rate_at_sale", "settle_currency_code", "tip_amount", "tip_method", "service_charge_amount", "service_charge_rate", "payment_method", "payment_status", "return_state", "status", "cash_session_id", "notes", "created_by", "checkout_started_at", "checkout_completed_at", "suspended_at", "suspended_by", "suspended_label", "reprint_count", "last_reprinted_at", "last_reprinted_by", "sync_status", "sync_version", "created_at", "updated_at" FROM `sales`;--> statement-breakpoint
...
CREATE INDEX `idx_sales_tenant_return_state` ON `sales` (`tenant_id`,`return_state`);--> statement-breakpoint
