...
export function ensureMigrationBaseline(sqlite: Database.Database, migrationsFolder: string): void {
  ...
  const shouldSeedPostBaselineMigration = (entry: DrizzleJournalEntry): boolean => {
    ...
    if (
      ...
      // parent and rebuild target, so executing it cannot produce a usable
      ...
      // fail at the absent products table. Pin it only for that complete
      ...
      // target in the same purchase-only adoption shape as 0059. Keep the
      // complete restaurant absence guard below: advancing past 0059 on a
      // mixed partial database would silently skip an applicable graph
      // migration merely because its sales table is absent.
      entry.tag === '0060_slimy_silver_centurion' ||
      ...
      // may pin the absent-target migration alongside 0061.
      entry.tag === '0062_wakeful_jack_murdock' ||
      ...
      return (
        ...
        (entry.tag !== '0060_slimy_silver_centurion' ||
          (!tableExists('restaurant_tables') &&
            ...
            !tableExists('restaurant_check_lines') &&
            !tableExists('restaurant_line_modifiers'))) &&
        (entry.tag !== '0061_bent_masque' || !tableExists('sales')) &&
        (entry.tag !== '0062_wakeful_jack_murdock' || !tableExists('sales')) &&
        (entry.tag !== '0063_clever_maddog' || !tableExists('devices')) &&
        !tableExists('product_search_fts') &&
        !tableExists('unit_x_product') &&
        !tableExists('products') &&
        !tableExists('ai_audit_log') &&
