/**
 * `observability.*` tRPC namespace (Web Vitals RUM).
 *
 ...
 * sample is gated by that tenant's telemetry opt-in (). The
 ...
 */
async function isTelemetryEnabled(db: DatabaseInstance, tenantId: string): Promise<boolean> {
  try {
    const row = await db
...
export const observabilityRouter = router({
  reportWebVital: publicProcedure.input(reportWebVitalInput).mutation(async ({ ctx, input }) => {
    // tenant is server-trusted: derive from the session, never
    // from client input. Null on anonymous (pre-login) page loads.
    const tenantId = ctx.tenantId;

    // Respect the per-tenant telemetry opt-in () when the tenant is
