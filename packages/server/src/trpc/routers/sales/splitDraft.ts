...
export const salesSplitDraftProcedures = {
  ...
  splitDraft: criticalCommandManagerOrAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // draft's sale number. saleSiteId is already proven non-null above, so
      // the helper's tenant-wide alphabetical fallback is unreachable here:
      // never fabricate financial provenance for a legacy draft whose
      // reservation site is unknown. Shared with the sale path rather than
      // duplicated — a second copy of this join is how the sites.tenantId
      // predicate and the deterministic fallback order get lost.
      const sequentialContext = await getSaleSequentialContext(ctx.db, ctx.tenantId, saleSiteId);
