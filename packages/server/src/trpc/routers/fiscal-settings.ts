...
import { validateNit } from '../../services/fiscal/packs/co/nit.js';
import {
...
export const fiscalSettingsRouter = router({
   ...
   */
  getByCountry: adminProcedure.input(getFiscalSettingsInput).query(async ({ ctx, input }) => {
   ...
   */
  updateMx: adminProcedure.input(updateMxFiscalSettingsInput).mutation(async ({ ctx, input }) => {
    ...
    if (input.rfc !== undefined) {
      partial.rfc =
        input.rfc === null
          ? null
          : validateRfc(input.rfc).ok && input.rfc
    ...
    const nextSettings = mergeMxFiscalSettingsIntoTenantSettings(tenantSettings, patch);
   ...
   * readiness fresco.
   */
  updateCl: adminProcedure.input(updateClFiscalSettingsInput).mutation(async ({ ctx, input }) => {
    ...
    if (input.rut !== undefined) {
      partial.rut =
        input.rut === null ? null : (validateRut(input.rut) as { normalized: string }).normalized;
    }
    ...
    if (input.casaMatriz !== undefined) {
      partial.casaMatriz = input.casaMatriz === null ? null : input.casaMatriz.trim();
    }
    if (input.environment !== undefined) {
      partial.environment = input.environment;
    }
    ...
    const nextSettings = mergeClFiscalSettingsIntoTenantSettings(tenantSettings, patch);
  ...
  updateCo: adminProcedure.input(updateCoFiscalSettingsInput).mutation(async ({ ctx, input }) => {
    ...
    // formato. Antes un NIT con formato correcto pero DV equivocado
    // (900373115-9) se guardaba y reventaba recién al emitir. `null`
    // = borrar (permitido); `undefined` = no tocar.
    if (input.nit !== undefined && input.nit !== null) {
      const result = validateNit(input.nit);
      if (!result.valid) {
        const message =
    ...
    if (input.dianResolutionNumber !== undefined) {
      partial.dianResolutionNumber =
        input.dianResolutionNumber === null ? null : input.dianResolutionNumber.trim();
    }
    if (input.prefix !== undefined) {
      partial.prefix = input.prefix === null ? null : input.prefix.trim().toUpperCase();
    }
    ...
    const nextSettings = mergeCoFiscalSettingsIntoTenantSettings(tenantSettings, partial);
    ...
    // ya almacenado.
    if (nextCo.rangeFrom !== null && nextCo.rangeTo !== null && nextCo.rangeFrom > nextCo.rangeTo) {
      throwServerError({
        trpcCode: 'BAD_REQUEST',
   ...
   */
  getActiveCaf: adminProcedure.input(getActiveCafInput).query(({ ctx, input }) => {
    if (input.countryCode !== 'CL') {
      return { caf: null };
    }
