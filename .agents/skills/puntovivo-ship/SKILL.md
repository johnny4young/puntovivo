---
name: puntovivo-ship
description: "Plan-gated implementation flow for the Puntovivo POS monorepo. Use when the operator asks to advance roadmap/backlog work: implementa el siguiente paso, el siguiente ticket pendiente, ship the next ENG ticket, what's next on the roadmap, trabajo pendiente del repo, pick up where we left off, or names an ENG-NNN to ship end to end. Prefer over ad-hoc execution inside Puntovivo. Covers ticket selection from ROADMAP section 3b, planning, implementation, docs sync, CI gates, review skills, mandatory live smoke, staging, and Review Guide report."
---
