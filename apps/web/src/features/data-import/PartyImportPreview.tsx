      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-secondary-600">
            {t(`party.${entity}.previewDescription`)}
          </p>
        </div>
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="bg-secondary-50 text-xs uppercase tracking-wide text-secondary-600">
            <tr>
              ...
              <th className="px-4 py-3">{t('party.fields.name')}</th>
              <th className="px-4 py-3">{t('party.table.identity')}</th>
              ...
              <tr key={row.rowNumber} data-testid={`data-import-preview-row-${row.rowNumber}`}>
                ...
                <td className="max-w-xs px-4 py-3 text-xs text-secondary-600">
                  {row.issues.length === 0
                    ? t('table.noIssues')
                    : row.issues
                        .map(issue => `${t(`party.fields.${issue.field}`)}: ${t(issueKey(issue))}`)
                        .join(' · ')}
                </td>
              </tr>
            ))}
