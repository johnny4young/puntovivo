...
[DESKTOP] [Electron] isPackaged: false, isDev: true, isStandalone: false  
...
The desktop app detects standalone mode via the `STANDALONE_MODE` environment variable:
...
if (isStandalone) {
  // Load from built files in workspace
