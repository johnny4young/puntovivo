...
| Controlled internal beta       | **Ready with release checks** | v1.10.1 published Developer ID signed and notarized macOS, signed Windows, and Linux artifacts with per-platform packaged smoke checks and a staged update feed. Cross-platform packaging and packaged encrypted recovery were then reproduced on the released tree itself, candidate `c6aebb8e`, in [run 31264233582](https://github.com/johnny4young/puntovivo/actions/runs/31264233582): structure, runtime and renderer smoke passed and encrypted recovery passed on Linux, macOS and Windows over the retained 262,865-row profile. Clean installation on representative machines, upgrade from the previous release and downgrade refusal are still operator-run checks and are not covered by that workflow, whose artifacts are intentionally unsigned; complete them before promoting the staged desktop rollout beyond its initial percentage. |
...
- Keep the v1.10.1 cross-platform packaging, runtime-smoke, signing,
  ...
  outstanding and are operator-run.
