...
| `ENG-031` | AI conversational analytics co-pilot | Pending | Tool-calling pattern via Vercel AI SDK: `runReadOnlySQL({ query: z.string() })` against a tenant-scoped read-only view + `getCurrentSiteContext()` enrichment tool. Frontend uses `@ai-sdk/react` `useChat` with `maxSteps:5` for multi-turn tool roundtrips. New page route `/co-pilot` admin/manager-only. Reuses `ENG-030` audit + budget infrastructure. | Demo conversation "¿cuánto vendí ayer en Sur?" returns SQL + result + chart inline; multi-tenant scoping pinned by integration test that switches tenants and verifies queries cannot cross. | **Medium** |
