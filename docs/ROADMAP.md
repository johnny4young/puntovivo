...
| 8 | Audit trail for sensitive actions | Required for operational trust — void, refund, price override, user change. Foundation **shipped**: `audit_logs` table + transactional `writeAuditLog` helper wired into `transfers.void`, `quotations.delete`, and `quotations.updateStatus` (convert only); admin-only `auditLogs.list` tRPC with filters (action / resourceType / resourceId / actor / date range); `/audit-logs` admin page with filter bar, translated action badges, per-action summary, CSV/Excel/PDF export. More audit points (sale.void, purchase.void, price override, user.disable) can be added by calling `writeAuditLog` from those services — no schema migration required. | Phase 8 |
