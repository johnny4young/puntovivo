...
| `ENG-018` | Sales park-and-resume (Iter 6) | Pending | Backend supports `status='draft'` on sales but the UI has no affordance. Build `sales.suspend`, `sales.resume`, `sales.listDrafts`, `sales.discardDraft` procedures with suspendedAt/suspendedBy/suspendedLabel columns and per-cashier lock (manager override). Add a Zustand `useCartWorkspace` persisted per user for multi-cart workflow, a `SuspendedSalesPanel`, a "Suspender" button in `SalesCheckoutPanel`, and `Ctrl+P` / `Ctrl+R` shortcuts with input-focus guard. Warn when closing a cash session with outstanding drafts. Full per-commit spec in [SPRINT-PLAN.md §3](./SPRINT-PLAN.md). | Cashier creates cart A, suspends, creates cart B, charges B, resumes A, charges A. Both complete. Stock decrements correctly per site. Two cashiers cannot resume the same draft concurrently. | **High** |
| `ENG-019` | Sales receipt reprint (Iter 7) | Pending | When a thermal printer runs out of paper or crashes, the cashier cannot reprint from history — the `print-receipt` IPC is one-way. Add `sales.getForReprint({ saleId, reason? })` with reprint counter (`reprintCount`, `lastReprintedAt`, `lastReprintedBy` on the sales row) + `sale.reprint` audit log. Surface a "Reimprimir" row action in `SalesHistoryPage`, a prominent reprint button in `SaleDetailsModal` with optional reason dropdown, a history banner when `reprintCount > 0`, and a `Ctrl+Shift+P` shortcut. Permissions: cashier reprints own shift; manager/admin reprints anything. Reuses the Iter 2 receipt renderer. Full per-commit spec in [SPRINT-PLAN.md §4](./SPRINT-PLAN.md). | Admin completes a sale, navigates to history, reprints; DB row shows `reprintCount = 1` and an audit row with `action = 'sale.reprint'` exists. Cashier cannot reprint another cashier's past-shift sale. | **High** |
