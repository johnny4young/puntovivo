...
| `ENG-027` | Dead code + dependency hygiene | Pending | Delete confirmed-unused symbols flagged by the 2026-Q2 code-quality audit (~12 named exports across `cash-session.ts:71,37`, `tenant-locale.ts:178`, `fiscal/registry.ts:26,31`, `template-expression.ts:791`, `templateUnavailableDecorations.ts:253`, `LocaleProvider.tsx:163`, `syncQueue.ts:32`, `useTableExport.ts:289`, `TableToolbar.tsx:322`, `test/utils.tsx:40,68,94,183,227`) plus drop the `@tanstack/react-virtual` dependency (zero references in `apps/web/src`). Default exports redundant with named exports get cleaned up in the same pass. | Each removal verified by 0-import grep across `apps/web/src` + `packages/server/src` + `apps/desktop/src/main`; `ci:web` + `ci:server` + `ci:desktop` green. | **Low** |
