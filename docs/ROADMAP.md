...
| `ENG-032` | AI anomaly + fraud detection | Pending | Local-only models (no LLM dependency): per-cashier z-score on tickets/hour, voids, no-sales; refund pattern outlier detection (isolation forest variant). New `services/ai/anomalyDetection.ts` (no AI SDK dependency — purely statistical). Surfaces in admin dashboard via tile + drill-down. Behind `ai.enabled` flag for consistency with `ENG-030` UX. | Synthetic anomaly fixture triggers alert; clean baseline does not; multi-tenant isolation tested; cashier role does not see other cashiers' anomalies (manager+ only). | **Medium** |
