...
| `ENG-028` | Cross-cutting helpers (mutate + sumBy + errorToast) | Pending | Three small helpers that collapse repeated boilerplate found by the 2026-Q2 audit: `apps/web/src/lib/mutationHelpers.ts::invalidateGroups(utils, [...keys])` (replaces 6 `Promise.all([...invalidate()])` blocks in `SalesPage.tsx` alone, plus duplicates in `purchases`, `customers`, `purchases/PurchaseDetailsModal`), `apps/web/src/lib/numbers.ts::sumBy(items, key)` (4 ad-hoc reducers in `SalePaymentModal.tsx`, `purchases/PurchasesPage.tsx`, `orders/OrdersPage.tsx`), `useMutationWithErrorToast()` wrapper around `useMutation` that injects `getErrorMessage(err, fallbackKey)` (27 files repeat the pattern). | Each helper used in at least 3 call sites with the old pattern removed; coverage stays ≥70/70/70/70. | **Medium** |
