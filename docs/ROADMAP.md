...
| `ENG-033` | AI semantic product search + auto-categorization | Pending | Embeddings via AI SDK `embed`/`embedMany` (Anthropic does not embed → falls to OpenAI `text-embedding-3-small` with provider gate); local cosine-similarity index in SQLite (extension or vector-blob column). Auto-categorization at product create uses `generateObject({ schema: z.object({ category: z.enum([...]) }) })` against the existing `categories` table. | "vino tinto reserva" matches without exact substring; new product gets pre-filled fiscal category with confidence above threshold; both behind `ai.enabled` flag and gated by `ai.settings.embeddings_provider` selection. | **Medium** |
| `ENG-034` | FISCAL-CORE refactor pluggable | Pending | Promote `services/fiscal/registry.ts` from implicit to a real `FiscalAdapter { resolve(saleSnapshot): Promise<FiscalDocument>; validateConfig(...): ValidationResult; emit(...): EmissionResult }` interface. Country packs become independent modules at `services/fiscal/packs/{co,mx,cl,...}`. Existing Colombia adapter migrates first to validate the contract. Touches `ENG-029` hotspot rule for `services/fiscal/` if a split is needed. | Existing CO behavior unchanged across `MockAdapter` + the planned `FactureAdapter` from `ENG-021`; adapter contract documented in `FISCAL-INTEGRATION.md`; new pack scaffold added (template). | **High** |
