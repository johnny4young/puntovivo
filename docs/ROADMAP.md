...
| `ENG-030` | AI-FOUNDATION (Vercel AI SDK + audit + settings) | Pending | Vercel AI SDK v5 wiring — adopt `ai` core + `@ai-sdk/anthropic` (default Sonnet 4.7); future-provider abstraction keeps the SDK stable when `@ai-sdk/openai` / `@ai-sdk/google` / Ollama are added later (Ollama is parked for `ENG-040`). New `services/ai/{provider,client,auditLog,tools/}.ts` server module with prompt-caching via `providerOptions.anthropic.cacheControl`, audit log table `ai_audit_log` (tenant_id, user_id, feature, model, tokens_in, tokens_out, cache_hit_tokens, cost_usd, duration_ms), per-tenant feature flags + monthly USD budget. New `ai.complete` / `ai.usage` / `ai.settings.*` admin tRPC procedures + `AISettingsPanel` UI. Single global `ai.enabled` flag for graceful degradation. Tests stub provider via `MockLanguageModelV1` from `ai/test`. | Audit log writes per call, budget cutoff enforced, multi-tenant isolation tested, role guards admin-only, `ai.enabled=false` short-circuits all features without errors. | **Medium** |
