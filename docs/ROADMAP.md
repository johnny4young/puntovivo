...
| 6 | Quotations / estimates | B2B, ferreterías, service businesses all need pre-sale conversion. Foundation **shipped** (`quotations` + `quotation_items` tables, `quotations.create/list/getById/updateStatus/delete` tRPC, dedicated Quotations page with status badges, transitions, and draft-only delete). Quote-to-sale conversion, version history, margin analysis, and follow-up reminders are deferred to later steps. | Phase 5 |
