...
> Status: **Accepted** (ENG-064 v1, 2026-05-05)
...
- `ENG-064b` (deferred) — Migrate the 19 router inline writers from `db.insert(syncQueue)` to `enqueueSync`; cut the existing 8 `sync.*` procedures over from `sync_queue` to `sync_outbox`; data-migrate any remaining legacy rows; drop `sync_queue` after a release cycle.
