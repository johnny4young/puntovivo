...
- `[security][deps]` Resolve the production `pnpm audit` high advisory on `tmp@0.2.6` pulled via `exceljs` in apps web/desktop; `pnpm run ci:server` currently stops at `ci:audit` with GHSA-7c78-jf6q-g5cm before server typecheck/lint/tests. Size as a dependency override or upstream bump after verifying ExcelJS compatibility. — 2026-06-15 (ENG-178 review blocker)
