...
- `[refactor][eng-178]` ENG-178 AC vs enumerated list: the ticket AC says no source file > 500 LOC, but the enumerated decompose list (and the per-slice tracker) omit several real >500 source files that the literal AC would require — `packages/server/src/db/seed-dev.ts` (2 134), `services/fiscal/orchestrator.ts` (1 190), `trpc/routers/sync.ts` (909), server `index.ts` (876), `trpc/routers/ai.ts` (864), `services/ai/copilot.ts` (861). Finishing the remaining tracked files (main/index.ts Slice 7, SalesPage 10b) will NOT satisfy the literal AC, so ENG-029 cannot truthfully close on those grounds. Operator decision still needed for the rest: fold these into ENG-178 (extends the wave) or promote a separate ENG. Note `seed-dev.ts` is a dev fixture — arguably exempt as "generated/seed data" — which the operator may want to carve out of the AC explicitly. — 2026-06-16 (ENG-178 slice 10 follow-up); `trpc/routers/products.ts` (1 280) folded into ENG-178 and shipped as slice 12 — 2026-06-17 (ENG-178 slice 12); `apps/web/src/features/sales/SalePaymentModal.tsx` (1 048) folded into ENG-178 and shipped as slice 13 — 2026-06-18 (ENG-178 slice 13). Operator approved the full remaining batch (geography, receipt-renderer, SalesPage 10b next, one commit each); `trpc/routers/geography.ts` (878) folded into ENG-178 and shipped as slice 14 — 2026-06-19 (ENG-178 slice 14). `services/receipt-renderer.ts` (1 204) folded into ENG-178 and shipped as slice 15 — 2026-06-19 (ENG-178 slice 15). `apps/web/src/features/sales/SalesPage.tsx` coupled-flow core (10b) extracted into `useSalesFlows.ts` and shipped as slice 16 — 2026-06-19 (ENG-178 slice 16); the operator batch (SalePaymentModal/geography/receipt-renderer/SalesPage 10b) is now complete, but SalesPage stays 940 LOC (still > 500) → see the slice-16b bullet below. Non-enumerated >500 set unchanged (SalesPage was a tracked/enumerated file, not part of that set); the rest of the fold-vs-new-ticket decision still pends. **2026-06-19 (slice 16b-1/16b-2)**: SalesPage finished — slice 16b-1 extracted useSalesCart + useSalesModals (940 → 732), slice 16b-2 extracted useSalesPageData + a presentational SalesScreen wrapper (732 → 421, < 500 closed). **2026-06-19 (non-enumerated split 1)**: operator chose to attack the non-enumerated SERVER set (ci:server-verifiable) over the no-live-smoke `main/index.ts`, FOLDING into ENG-178 (decision resolved: fold, not a new ENG). `trpc/routers/sync.ts` (909) decomposed into `routers/sync/{index,helpers,status,queue,conflicts,push,contract}.ts` (sibling-record) — DONE. **`seed-dev.ts` (2 134) decision: AC-EXEMPT** as a hand-written dev fixture / CLI script (refuses to run under `NODE_ENV=production`, not shipped runtime) — default applied, pending operator confirmation. **2026-06-19 (split 2)**: `trpc/routers/ai.ts` (864) decomposed into `routers/ai/{index,helpers,settings,copilot,anomalies,invoiceOcr,standalone}.ts` (sibling-record, nested sub-routers preserved) — DONE; the FAT `invoiceOcr.extract` (190 LOC) stays inline (deferred follow-up: extract to `application/ai/invoiceExtraction.ts`). Remaining non-enumerated RUNTIME files to fold: `services/fiscal/orchestrator.ts` (1 190, fiscal-sensitive), server `index.ts` (876, Fastify bootstrap), `services/ai/copilot.ts` (861); plus `apps/desktop/src/main/index.ts` (1 769, Slice 7, the no-live-smoke Electron half). ENG-029 closes once those land.
- `[refactor][eng-178]` SalesPage slice-16b-2 SHIPPED (2026-06-19): `SalesPage.tsx` 732 → 421, closing the < 500 AC for this file. NOTE the approach changed from the slice-16b-1 plan: a fresh Plan-agent LOC audit refuted the props-bundle lever (it lands ~555 + turns `useSalesModals` into a prop-aggregator god-hook, since slices 16/16b-1 already harvested the handler bodies). The only single lever to cross < 500 was moving the JSX out, so the slice extracted `useSalesPageData.ts` (the read side) + a presentational `SalesScreen.tsx` wrapper (relocation, in-precedent: PosTouchScreen/*Shell). ENG-178 stays Partial overall: `main/index.ts` (Slice 7) + the non-enumerated >500 set still remain.
