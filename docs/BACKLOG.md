...
- `[infra][desktop]` End the better-sqlite3 ABI swap dance: keep BOTH cached binaries (node + electron, already compiled by ensure-native-runtime) and select per-runtime via the constructor nativeBinding option in db/index.ts (process.versions.electron ? electron copy : node copy; fall back to default resolution when the cache is missing or app.isPackaged). Kills the file swap, the stale-marker class (ERR_DLOPEN_FAILED after a desktop boot) and every rebuild beyond the first compile per ABI. argon2 is already N-API so better-sqlite3 is the only swapped module. — 2026-06-10 (auditoria follow-up)
- `[infra][tooling]` electron-stagewright: 3 findings from the audit smoke (window-handle loss after a modal confirm → INTERNAL_ERROR firstWindow timeout; electron_stop exhausting the 120s dispatch timeout on a hung app instead of force-killing; snapshot diffs exceeding the MCP token cap despite maxEntries). Drafted in .audit/stagewright-limitations.md, pending operator confirmation before writing to that repo's backlog. — 2026-06-10 (auditoria follow-up)
