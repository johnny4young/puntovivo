...
- `[sales][e2e][layout]` Pre-existing since ENG-186 (`ad07d30`), surfaced during ENG-134g: the cash-session close + manual cash-movement E2E tests (`business.spec.ts:870/942/1012/1302`) fail at the 1280x720 Playwright viewport — the ENG-186 `xl:shrink-0` sticky checkout footer (`SalesCheckoutPanel.tsx:374` + `.pv-total`) intercepts pointer events over the Record movement button. Reproduces with the surface modules OFF (cleared `local.db` + disabled the seed helper), so it is NOT caused by ENG-134g; it went unnoticed because the Playwright suite is local-only, not push-CI. 1280x720 / 1366x768 are common cashier resolutions, so this may be a real operator bug rather than a test artifact — triage whether to fix the checkout-panel overlap (give the scroll region the room, or drop the sticky footer below `xl`) or raise the e2e viewport if it is purely a test-viewport issue. Likely warrants promotion to an `ENG-NNN` since it blocks a critical POS flow in the local suite. — 2026-06-09 (ENG-134g follow-up)
