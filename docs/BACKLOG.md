...
- `[refactor][eng-178]` SalesPage slice-16b: drive `SalesPage.tsx` from 940 LOC to < 500. Slice 16 extracted the coupled sale-lifecycle flow core into `useSalesFlows.ts`; the shell still holds the cart-edit handlers (`getCartSummary`/`mergeCartItem`/`updateCartItem` wiring + quantity/discount/remove), the suspended-panel + history-reprint nav handlers, the keyboard-shortcut map, and the large inline cart/checkout JSX. Remaining decomposition: lift the cart-edit handlers into a `useSalesCart`-style hook and extract the inline JSX into the slice-10 section components (`SalesCartSection` / `SalesCheckoutSection` already named in the ENG-178 enumeration). Behavior-preserving (React Compiler OFF → preserve every memoization + dep array verbatim); the `SalePaymentModal.test.tsx` + a live smoke are the net. — 2026-06-19 (ENG-178 slice 16 follow-up)
