...
| `ENG-038` | LATAM payment rails (Wompi + Bold + ePayco + Mercado Pago + Nequi/Daviplata) with AI-assisted nightly reconciliation. **Partial 2026-05-11**: shipped rail manifest + deterministic adapters, `payment_outbox`, read-only `payments.*` tRPC and `/operations?tab=payments`; real provider workers, credentials and nightly matching proof remain. |
