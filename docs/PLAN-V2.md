...
| Ticket | Scope summary |
| --- | --- |
| `ENG-039` | Vertical restaurant (tables, KDS, tips, modifiers; the México-only CFDI surface stays as a Gated child via `ENG-039f`) — vector against SoftRestaurant's legacy stack. The umbrella vertical is multi-country LATAM; only `ENG-039f` (CFDI MX issuance) is México-specific because it depends on `ENG-035c`'s PAC contract. First actionable slice was `ENG-039a` (voice ordering); `ENG-039b` shipped the table catalog; `sales.tableId` FK + open-table state machine + tip / service charge / split bill landed across `ENG-039c..ENG-039d3`; KDS now lives in `ENG-098`; per-line modifiers in `ENG-039d2`. Only `ENG-039f` (CFDI MX issuance) remains, Gated on the PAC contract. |
...
| `ENG-039c2` | Restaurant transfer-to-table CTA on `SuspendedSalesPanel` — wires the manager/admin-only `sales.changeTable` into a "Cambiar mesa" button per draft + new `TransferTableModal` with `restaurantTables.listWithDraftStatus` as the dropdown source (first consumer of that ENG-039c read surface). **Shipped 2026-05-14** — split-bill stays deferred to ENG-039c3; modifiers / tips / KDS / CFDI MX remain in their own child slices. |
| `ENG-039c3` | Restaurant split-bill mutation + UI — new `sales.splitDraft` manager/admin critical-command that carves a subset of `sale_items` out of a suspended draft into a fresh suspended draft (stock-neutral: items relocated, not re-debited) with optional `tableId` for the new draft; new `SplitBillModal` off `SuspendedSalesPanel` lets the operator tick which items move + pick the target table or "Liberar mesa" + free-text label. **Shipped 2026-05-14** — modifiers / tips / KDS / CFDI MX remain in their own child slices. Collateral: `getSaleRecord` now exposes `tableId` (was missing since ENG-039c) and ADR-0002 closed list refreshed to include `sales.changeTable` + `modules.setActive` + `sales.splitDraft`. |
| `ENG-039d` | Restaurant tip / propina capture — sixth restaurant slice. First of three concerns originally bundled into the umbrella's `ENG-039d` line (modifiers and service charge math re-promoted to `ENG-039d2` + `ENG-039d3`). **Shipped 2026-05-14** — new migration `0025_sales_tip.sql` adds `tip_amount` + `tip_method` to `sales`, Zod schemas + `completeSale` use-case roll tip into total on both fresh and fromDraft paths, `SalePaymentModal` gains a Propina section with 0% / 10% / 15% presets + custom amount, `SalesPage` threads the new fields through both mutations, and the existing `sale.complete` audit row picks up `tipAmount` / `tipMethod` metadata when non-zero. Refund flow unchanged in v1 (refund returns `total` which now includes tip). |
| `ENG-039d3` | Restaurant service charge / propina sugerida math — seventh restaurant slice. Sibling to `ENG-039d` (tip): a tenant-configured percentage that auto-applies on every checkout for restaurant-mode tenants (mandatory + non-discretionary, not a voluntary preset). **Shipped 2026-05-14** — new migration `0026_sales_service_charge.sql` adds `service_charge_amount` + nullable `service_charge_rate` to `sales`; new `services/restaurant/settings.ts` mirrors the AI client over `tenants.settings.restaurant.serviceChargeRate`; `completeSale` extends both fresh + fromDraft paths to `subtotal + tax - discount + tip + service` (fromDraft recomputes `baseTotal` from frozen pieces so a draft-time charge does NOT compound at completion). Two new error codes (`SALE_SERVICE_CHARGE_DRIFT`, `SALE_SERVICE_CHARGE_DISABLED`) protect retail tenants. New `restaurantSettings.{get, update}` tRPC router + admin Company tab `/company?tab=restaurant` rendering `CompanyRestaurantSettingsCard` (single 0–30 input). `SalePaymentModal` reads the rate via a new prop and renders a read-only "Servicio" section when > 0, folding the amount into the grand total with three breakdown i18n keys (tip-only / service-only / both). New `payment.serviceCharge.*` + `company.restaurant.*` keys in en + es neutral LATAM tú. 11 new server tests + 4 modal cases + 3 settings-card cases. Refund flow unchanged in v1 (refund returns `total` which now includes both tip + service). |
