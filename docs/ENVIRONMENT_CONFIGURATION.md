...
| `PUNTOVIVO_HUB_URL`                | unset                                                                            | Hub URL when `PUNTOVIVO_AUTHORITY_MODE=hub_client`. — Electron main exposes this to the renderer via the `runtime:get-config` sync IPC; `apps/web/src/lib/trpc.ts` resolves the tRPC base to this URL at module init. Required in `hub_client` mode (without it the resolver falls back to the `VITE_API_URL` default and the terminal silently talks to whatever `http://localhost:8090` resolves to). |
...
| `PUNTOVIVO_ALLOWED_LAN_ORIGINS`    | unset                                                                            | Comma-separated CORS origins accepted in `site_hub` mode. Reserved for .                                                                                                                                                                                                                                                                                                                                |
