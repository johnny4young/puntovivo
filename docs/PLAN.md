...
- [`docs/A11Y.md`](./A11Y.md) — accessibility contract: axe-core helper in `apps/web/src/test/a11y.ts` runs WCAG 2 A + AA on component tests, `scripts/check-contrast.mjs` enforces token contrast ratios in `ci:web`, plus the ARIA conventions reviewers check on every UI PR (`ENG-134`)
