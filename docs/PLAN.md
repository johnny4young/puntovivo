...
- Migrations `0004_dian_identification_types.sql` + `0005_fiscal_documents.sql` land the global 10-code DIAN ID catalog plus the 4 tenant-scoped fiscal tables. Buyer (`buyerTaxId`, `buyerName`, `buyerAddress`, 5 more) and line (`productName`, `productSku`, `unitMeasureCode`, 7 more) snapshot columns are frozen at emission — they must not be updated thereafter. Migration `0038_eng176c_fiscal_identification_catalog.sql` (ENG-176c, 2026-05-26) renames the catalog to `fiscal_identification_types` with composite PK `(country_code, code)` and seeds MX/PE/CL rows alongside DIAN; `fiscal_documents` gains `buyer_country_code` so the composite FK resolves per-country.
...
### 17.0 Status Update — May 2026 (ENG-176b currency seam on transactional tables)

**ENG-176b shipped**. Migration `0037_eng176b_currency_seam.sql` extends the locale work above with a per-row currency seam on every monetary transactional table. `tenants.default_currency_code` (TEXT NOT NULL, FK `currency_catalog.code`, DEFAULT `'COP'`) is the canonical write target; back-filled by the migration via COALESCE(`tenant_locale_settings.currency_override`, country default via `tenant_locale_settings.country_code`, `json_extract(tenants.settings, '$.currency')`, `'COP'`). Sales, sale_items, quotations, and quotation_items each get `currency_code` + `exchange_rate_at_sale` + nullable `settle_currency_code` (plus a `chk_*_exchange_rate_positive` CHECK), products gets `currency_code`, and customers gets `credit_limit_currency_code` (nullable; set only when `creditLimit > 0`). Helper `packages/server/src/lib/currency.ts:resolveTenantCurrency(db, tenantId)` reads the tenant default in O(1) and is consumed by `completeSale.ts`, `services/quotations.ts`, and the products / customers tRPC routers so every monetary write stamps a currency. The same migration finally attaches the deferred `_nonneg` + `_2dec` CHECKs to `fiscal_documents`, `fiscal_document_items`, and `payment_outbox` — the three tables the Drizzle snapshot chain had skipped during Step-a/Step-b. Existing receipt + reporting surfaces continue to work unchanged because they read currency from the tenant locale; the per-row seam is forward-looking and only matters once a tenant operates in more than one currency (ENG-156 multi-currency, ENG-161 NFe Brazil).

### 17.0 Status Update — May 2026 (ENG-176c fiscal identification catalog + status enum expansion — ENG-176 fully closed)

**ENG-176c shipped**. Migration `0038_eng176c_fiscal_identification_catalog.sql` renames `dian_identification_types` → `fiscal_identification_types` with composite PK `(country_code, code)` so DIAN / SAT / SUNAT / SII codes coexist without collision in the same global table. `fiscal_documents` gains `buyer_country_code TEXT NOT NULL DEFAULT 'CO'` with FK to `country_catalog`; the identification FK shifts to a composite reference `(buyer_country_code, buyer_tax_id_type_code) → fiscal_identification_types(country_code, code)`. Legacy DIAN rows back-fill to `country_code = 'CO'`; legacy fiscal_documents rows back-fill `buyer_country_code = 'CO'`. The boot seed (`db/index.ts:seedFiscalIdentificationTypes()`) grows from 10 to 23 rows by adding minimal viable subsets for SAT México (RFC, CURP, IFE, PA), SUNAT Perú (DNI, RUC, CE, Pasaporte, no domiciliado), and SII Chile (RUT, RUN, EXT, Pasaporte). `fiscalDocumentStatusEnum` extends from 5 DIAN-native states to 8 (adds `voided` for SAT/SII/NFe cancelaciones, `notified_correction` for SAT acuses, `partial_send` for SUNAT lotes); `FiscalStatusBadge.tsx` mirrors the union with tone mapping (`voided` → danger, `notified_correction` → warning, `partial_send` → primary) and `fiscal:status.*` i18n keys land in EN + ES (neutral LATAM register). Adapter integration: the single catalog-lookup site in `services/fiscal/orchestrator.ts` filters by `(countryCode, code)` against the composite PK; the orchestrator hard-codes `buyerCountryCode = 'CO'` until ENG-156 / ENG-161 wire per-tenant country routing. The ENG-176 thread closes Shipped end-to-end — money storage precision (Step-a + Step-b), currency seam (ENG-176b), and identification catalog + status enum (ENG-176c). ENG-156 multi-currency operations + ENG-161 NFe Brazil can now begin without further schema prep.
...
- [`docs/architecture/0005-sync-payload-contract.md`](./architecture/0005-sync-payload-contract.md) — per-entity sync payload contract + manifest + version-bump protocol locked before ENG-065 / ENG-066 / ENG-068+ build on the new `sync_outbox` shape (**shipped** as `ENG-064` v1; cutover + drop of legacy `sync_queue` shipped as `ENG-064b` via migration `0017_drop_sync_queue.sql`; `hardware_outbox` reaches idempotency parity via migration `0018_hardware_outbox_idempotency.sql` shipped as `ENG-067b`)
- [`docs/architecture/0006-local-data-security.md`](./architecture/0006-local-data-security.md) — local data security threat model: atomic backup via SQLite online-backup API + integrity-checked restore with corruption rejection; ZIP bundle that travels device identity; recursive sanitizer at the diagnostic-export boundary with anchored denylist; schema-level PAN/CVV column lint; per-OS keychain seam documented for ENG-063's payment integration (**shipped** as `ENG-066`)
- [`docs/architecture/0007-module-activation.md`](./architecture/0007-module-activation.md) — module activation kernel: per-tenant module state in `tenants.settings.modules` (no new table); manifest at `services/modules/manifest.ts` is the single source of truth; `createModuleGuard(moduleId)` server middleware + `RequireModule` renderer wrapper gate by id; soft-disable semantics — deactivated modules return FORBIDDEN with `MODULE_NOT_ACTIVATED` and rows persist; 5 demo modules wired (copilot, operations-center, quotations, anomaly-detection, semantic-search) as the foundation for ENG-069 (multi-surface POS shell) and ENG-070 (event-based public API) (**shipped** as `ENG-068`)
- [`docs/AUTHORITY-NODE.md`](./AUTHORITY-NODE.md) + [`docs/architecture/0008-authority-node-runtime-modes.md`](./architecture/0008-authority-node-runtime-modes.md) — Authority Node runtime modes: default `device_local`, configurable `site_hub`, non-authoritative `hub_client`, and future central-server ingestion downstream of local outboxes/events (**shipped** as `ENG-071`)
- [`docs/architecture/0009-money-storage-and-validation.md`](./architecture/0009-money-storage-and-validation.md) — money storage + validation policy: SQLite `real` columns + schema-level `>= 0` CHECK invariants (chosen over integer-minor-units migration); Step-b adds the shared `roundMoney()` helper plus two-decimal precision CHECKs on covered money tables; per-currency exponent honored once ENG-176b adds `currency_code` to transactional rows (**shipped** as `ENG-176a` Step-a + Step-b)
- [`docs/PLAN-V3.md`](./PLAN-V3.md) — tactical bridge for the World-Class LatAm POS wave (`ENG-103..ENG-165`): market objective, adoption, launch migration, cashier UX, loyalty, omnichannel, money trust, procurement, accounting, owner BI, KDS v2, public API delivery, supportability, privacy, AI automation, vertical-depth packs, information architecture simplification, progressive disclosure, performance budgets, accessibility AA, production observability, backup/restore, auto-update, subscription/billing, SSO/MFA, shift management, day-close report, loss-prevention rules, in-transit transfers, WhatsApp inbound commerce, tip pooling, recipe scaling, self-checkout, hardware compatibility matrix, shared peripherals, universal print server, chaos/DR, data portability, cohort/LTV/RFM, receipt branding, contextual help, multi-currency, public demo sandbox, tenant clone, data residency, Nómina Electrónica DIAN (Gated), pt-BR + NFe Brazil, hosted micro-storefront, compliance readiness pack, hosted-SaaS substrate spike (`ENG-164`), and tRPC-aware rate limiting (`ENG-165`); replaces H0..H4 horizons with a single flat priority list
- [`docs/PERF-BUDGETS.md`](./PERF-BUDGETS.md) — performance budget contract for the repo: bundle-size + tRPC p95 latency gates wired into `ci:web` / `ci:server`, baselines + thresholds anchored on the root `perf-budget.json`, and the playbook for bumping baselines deliberately (`ENG-133`)
- [`docs/A11Y.md`](./A11Y.md) — accessibility contract: axe-core helper in `apps/web/src/test/a11y.ts` runs WCAG 2 A + AA on component tests, `scripts/check-contrast.mjs` enforces token contrast ratios in `ci:web`, `e2e/web/a11y.spec.ts` runs the Playwright a11y smoke against the top user-facing routes in the `e2e-web` job, plus the ARIA conventions reviewers check on every UI PR (`ENG-134`)
- [`docs/ACCESIBILIDAD.md`](./ACCESIBILIDAD.md) — Spanish-language accessibility commitment doc for pre-sales, auditors, and non-technical reviewers: WCAG 2.2 AA promise in plain language, regulatory backing for LATAM (Ley 1346 in CO, NMX-R-050 in MX), and the three technical gates that back the promise (`ENG-134`)
- [`docs/OBSERVABILITY.md`](./OBSERVABILITY.md) — production observability rail: `packages/server/src/observability/` sink interface + captureException / withSpan helpers, tRPC tracing middleware that stamps `procedure / durationMs / outcome / correlationId / tenantId / userId`, per-tenant opt-in flag (`tenants.settings.telemetryOptIn`) + audit row, renderer-side `captureRenderError` wired to `AppErrorBoundary` and `window.error / unhandledrejection`, plus the adapter wiring recipe the follow-up Sentry / GlitchTip install will use (`ENG-135`)
- [`docs/SHORTCUTS.md`](./SHORTCUTS.md) — canonical keyboard-shortcut catalogue + global Command Palette UX: `apps/web/src/lib/shortcuts.ts` is the declarative source of truth, the `Mod+K` palette in `CommandPalette.tsx` consumes it for action hints, real `/sales` buttons consume it for `aria-keyshortcuts`, and the doc lists every catalogue entry plus the recipe for adding a new shortcut or palette action (`ENG-105` slice A)
- [`docs/MARKET-SEGMENTS.md`](./MARKET-SEGMENTS.md), [`docs/MODULE-ACTIVATION.md`](./MODULE-ACTIVATION.md), [`docs/FUTURE-VERTICALS.md`](./FUTURE-VERTICALS.md), [`docs/LATAM-EXPANSION.md`](./LATAM-EXPANSION.md), [`docs/LONG-TERM-VISION.md`](./LONG-TERM-VISION.md), [`docs/STACK-EVOLUTION.md`](./STACK-EVOLUTION.md), [`docs/SALES-COCKPIT.md`](./SALES-COCKPIT.md), [`docs/UI-SURFACES.md`](./UI-SURFACES.md) — strategy + architecture evolution
- [`docs/SECURITY.md`](./SECURITY.md) — security posture doc, expanded under `ENG-166` (helmet/CSP, Argon2 wrapper, cookie sameSite strict, login timing equality, per-procedure rate-limit subset, print HTML sanitiser, devtools env gate, trustProxy deployment contract)
- [`docs/AUDIT-2026-05-24.md`](./AUDIT-2026-05-24.md) — 2026-05-24 cross-cutting audit (security + frontend perf + database/Drizzle + code quality) that opens `ENG-166..ENG-181`; Wave 1 (`ENG-166`) marked closed
