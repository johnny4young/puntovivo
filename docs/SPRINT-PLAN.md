...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (2 / 21 closed; ENG-052 Partial-foundation) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index) y `ENG-051` (Foundation ADR pack). **In flight**: `ENG-052` Partial — ENG-052a foundation shipped (schema 0010/0011 con `devices` + `idempotency_keys` + reserva atómica, `auth_logs.operation_id`; servicios puros `devicesService` + `idempotencyService` + `keyHasher`; middleware `commandEnvelope` + decorator `criticalCommandProcedure`; procedure `auth.registerDevice`; `auth.changePassword` migrado a `criticalCommandProcedure` como proof end-to-end; web `deviceId.ts` + `commandEnvelope.ts` + `useCriticalMutation('auth.changePassword')` + AuthProvider register-device; CORS allow x-device-id + x-puntovivo-envelope; 42 tests nuevos verde + 5 changePassword tests adaptados; smoke Playwright verde). **ENG-052b remaining**: wire 17 procedures restantes de la closed list ADR-0002 + adapta sus test files + expandir `useCriticalMutation` para esas mutaciones + Electron preload `device.getId/setId` + Fastify request-scoped logger hook. Next después de cerrar ENG-052b: `ENG-053..ENG-056` journal/sale lifecycle/cash boundary, seguidos por fiscal readiness (ENG-057..ENG-058 consume el `fiscal_outbox` definido en ADR-0003), hardware readiness, sync/operations/resilience, y expansion architecture. |
