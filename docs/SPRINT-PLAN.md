...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (5 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index), `ENG-051` (Foundation ADR pack), `ENG-052` cerrado completo en dos slices — `ENG-052a` foundation (schema 0010/0011, `devices` + `idempotency_keys` + reserva atómica, middleware `commandEnvelope`, `auth.changePassword` como proof) y `ENG-052b` close-out (17 procedures críticas + 9 test files adaptados + `useCriticalMutation` generalizado + Electron preload + Fastify request-logger hook), `ENG-053` (operation journal + outbox kernel — migración 0012 con `operation_events` + `operation_effects` + `operation_errors` + `outbox_metadata`; servicio `services/operation-journal/` y kernel `lib/outbox/` con factory `createOutboxKernel<TStatus,TPayload>` + worker base `tickOutbox` + retry policy `BOUNDED_EXPONENTIAL_BACKOFF` + atomic claim via `claim_token`; `commandEnvelope` encadena `recordOperationStart/markOperationCompleted/recordError`; pattern docs nuevos en `architecture/patterns/operation-journal.md` y `architecture/patterns/outbox-kernel.md` con sección Patterns en `architecture/README.md`; 34 tests nuevos verde, ci server 882/882, smoke vía Playwright valida success/replay-cached/failure trail), y `ENG-054` (extract `completeSale` application service — nuevo directorio `packages/server/src/application/` con README documentando la convención y primer feature `application/sales/` que aterriza el use-case `completeSale(ctx, input)` cubriendo los DOS paths `{ mode: 'fresh' \| 'fromDraft' }`. Policies puras, `getSaleRecord`, y `safelyEmitFiscalDocument` reubicados. `sales.create` y `sales.completeDraft` reducidos a thin wrappers; primer service-level consumer del journal de ENG-053 — `operation_effects` se pueblan automáticamente en cada venta. 13 invariant tests sin tRPC + 45 tests existentes verde sin modificación. Server coverage 83.65/69.37/80.20/84.76. Live smoke con seed mega: 46 sale_row + 60 payment_row + 84 inventory_movement + 46 cash_movement + 46 sync_queue_emit + 46 fiscal_emit). Next: `ENG-055..ENG-056` sale lifecycle services + cash session aggregate boundary, seguidos por fiscal readiness (ENG-057..ENG-058 consume el `fiscal_outbox` definido en ADR-0003 vía el kernel ENG-053), hardware readiness, sync/operations/resilience, y expansion architecture. |
