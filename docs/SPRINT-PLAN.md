...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (3 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index), `ENG-051` (Foundation ADR pack), y `ENG-052` cerrado completo en dos slices — `ENG-052a` foundation (schema 0010/0011 con `devices` + `idempotency_keys` + reserva atómica, servicios puros `devicesService` + `idempotencyService` + `keyHasher`, middleware `commandEnvelope` + decorator `criticalCommandProcedure`, `auth.registerDevice`, `auth.changePassword` como proof end-to-end, web `deviceId.ts` + `commandEnvelope.ts` + `useCriticalMutation('auth.changePassword')` + AuthProvider register-device) y `ENG-052b` close-out (17 procedures críticas envueltas con sus variantes `criticalCommand*` en sales lifecycle / cashSessions / inventory / transfers / users; 9 test files adaptados con Headers Proxy de envelope fresco por acceso; `useCriticalMutation` generalizado con `CriticalCommandPath` + type inference automático; 18 call sites en SuspendedSalesPanel/SaleDetailsModal/SalesPage/InventoryPage/InventoryTransferHistory/InventoryBalancesPanel/UsersPage migrados; Electron preload `electron.device.getId/setId` + main IPC `device:get-id`/`device:set-id` con atomic write a `app.getPath('userData')/device-id.txt`; Fastify `onRequest` hook con `requestId`/`deviceId` colgado de `request.log`; `commandEnvelope` middleware encadena su child off `req.log`; tres test files nuevos — `request-logger.test.ts`, `device-id-store.test.ts`, `useCriticalMutation.test.tsx`). Next: `ENG-053..ENG-056` journal/sale lifecycle/cash boundary, seguidos por fiscal readiness (ENG-057..ENG-058 consume el `fiscal_outbox` definido en ADR-0003), hardware readiness, sync/operations/resilience, y expansion architecture. |
