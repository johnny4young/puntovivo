...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (2 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index) y `ENG-051` (Foundation ADR pack — `docs/architecture/` con `README.md` + cuatro ADRs lockeando Local Store Authority, Command Envelope, Outbox Taxonomy y Conflict Policy; secciones fiscales en español por convención AGENTS.md). Next: `ENG-052..ENG-056` device/idempotency/journal/sale lifecycle/cash boundary (envelope shape + closed list de comandos críticos ya lockeados por ADR-0002), seguidos por fiscal readiness (ENG-057..ENG-058 consume el `fiscal_outbox` definido en ADR-0003), hardware readiness, sync/operations/resilience, y expansion architecture. |
