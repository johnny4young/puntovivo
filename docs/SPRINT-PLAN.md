...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (8 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index), `ENG-051` (Foundation ADR pack), `ENG-052` (device registry + command envelope, two slices), `ENG-053` (operation journal + outbox kernel — migración 0012 + factory `createOutboxKernel<TStatus,TPayload>` + worker base `tickOutbox` + `BOUNDED_EXPONENTIAL_BACKOFF`), `ENG-054` (extract `completeSale` application service), `ENG-055` (extract sale lifecycle services — `returnSale`, `voidSale`, `discardDraft`), `ENG-056` (cash session aggregate boundary — `openCashSession` / `closeCashSession` / `recordCashMovement` use-cases + new `cashSessions.pendingChecks` query + `insertCashMovement` widened to full `CashMovementType`), y `ENG-057` (fiscal outbox + contingency engine — migración `0013_fiscal_outbox.sql` con primera tabla concreta consumiendo el outbox kernel; 7-state status enum + `kind=['emit']` + 3 indexes; closed `NormalizedFiscalErrorKind` union de 10 kinds en `services/fiscal/errors.ts` + `error-normalizer.ts` con default heuristic + 3 stub per-pack mappers; `enqueueFiscalEmission` reemplaza el silent-failure path — pre-create `fiscal_documents` con `status='pending'` + CUFE placeholder + enqueue outbox row en una tx, validateConfig pre-flight para paquetes parqueados; back-compat shim `safelyEmitFiscalDocument` preserva firma — los tres callers no requirieron edits; nuevo daemon `services/fiscal/fiscal-worker.ts` boot wired en `index.ts::createServer` con setInterval 30s + immediate tickOnce post-enqueue + stale-claim sweep + per-tick adapter timeout 60s; mirror invariant: doc.status updated BEFORE kernel transition; admin `reports.fiscal.retryDocument` re-arma o re-enqueue stuck rows; test seam `__setFiscalAdapterForTest` + MockAdapter `throwOracle`; 6 integration tests + 4 acceptance paths verificados + retry dead-letter idempotente post-accepted; server 969 tests verde, coverage 83.6/69.7/79.89/84.78). Next: `ENG-058` receipt fiscal finalization (CUFE/QR/XML/status rendering en receipts), hardware readiness, sync/operations/resilience, y expansion architecture. |
