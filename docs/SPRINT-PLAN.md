...
| Out-of-band | [`ENG-042`](./ROADMAP.md) | Partial | Sync resilience UX — stale conflict handling + network error display + entity labels humanos. Server `sync.listConflicts` / `sync.pull` exponen `localRecordExists`; `sync.resolve` rechaza keepLocal/merged cuando el local row falta. Web `companySyncDisplay.ts` mapea entityType + operation a labels i18n; conflict modal gana flow "Discard Local Change". `translateServerError.isNetworkConnectivityError` silencia los `Failed to fetch` al boot del AuthProvider y los traduce a `errors:server.networkUnavailable`. **Remaining**: 2 follow-ups en BACKLOG bajo working title `ENG-043` — TOCTOU (mover el `findEntity` guard dentro del `db.transaction`) + reemplazar el `error.message` prose por `errorCode: 'SYNC_LOCAL_RECORD_MISSING'` con locale copy. |
