...
4. **`ENG-030..ENG-033` — AI Wave 1** — `ENG-030` and `ENG-031` are shipped. Next eligible work is `ENG-032` (local-only anomaly) or `ENG-033` (semantic product search, still dependent on provider/embedding choices). Ollama remains parked for `ENG-040`.
