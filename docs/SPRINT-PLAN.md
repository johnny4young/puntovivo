...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (1 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index). Next: `ENG-051` ADR pack, then `ENG-052..ENG-056` device/idempotency/journal/sale lifecycle/cash boundary, followed by fiscal readiness, hardware readiness, sync/operations/resilience, and expansion architecture. |
...
1. **`ENG-051` — Foundation ADR pack**. Locks the architecture decisions required before mutating schema or extracting services: Local Store Authority, Command Envelope, Outbox taxonomy, and Conflict policy.
2. **`ENG-052..ENG-056` — Foundation Reset core**. Device registry + idempotency, operation journal + outbox kernel, `completeSale` extraction, sale lifecycle extraction, and cash-session aggregate boundary. Run strictly in order.
