...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (10 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index), `ENG-051` (Foundation ADR pack), `ENG-052` (device registry + command envelope, two slices), `ENG-053` (operation journal + outbox kernel — migración 0012 + factory `createOutboxKernel<TStatus,TPayload>` + worker base `tickOutbox` + `BOUNDED_EXPONENTIAL_BACKOFF`), `ENG-054` (extract `completeSale` application service), `ENG-055` (extract sale lifecycle services — `returnSale`, `voidSale`, `discardDraft`), `ENG-056` (cash session aggregate boundary — `openCashSession` / `closeCashSession` / `recordCashMovement` use-cases + new `cashSessions.pendingChecks` query + `insertCashMovement` widened to full `CashMovementType`), `ENG-057` (fiscal outbox + contingency engine — migración `0013_fiscal_outbox.sql`, kernel-driven worker daemon, status mirror invariant; admin `reports.fiscal.retryDocument` para manual recovery), `ENG-058` (receipt fiscal finalization — Option A surgical fix: nuevo `services/fiscal/qr-builder.ts` con per-country QR payload builders + `isPlaceholderCufe` defense-in-depth helper, `RenderFiscal` extendido con `status`+`statusLabel`, `getSaleRecord` widened para devolver `fiscalDocuments[]` con multi-doc query cubriendo DEE/FEV + void/return NCs, nuevo `FiscalStatusBadge` shared component reusando `fiscal:status.*` i18n keys, nuevo `SaleDetailsFiscalBlock` montado en `SaleDetailsModal` con conditional CUFE display + verify link + admin Ver XML, `receiptPrinter.ts` extendido con `buildFiscalSection` async + dynamic-imported `qrcode` library 16KB lazy chunk, nuevo namespace `receipts.json` en/es; status copy es la SINGLE SOURCE OF TRUTH; tests cubren acceptance criterion con NEGATIVE REGEX `/receipt-fiscal[\s\S]*Accepted/` para garantizar que contingency nunca renderiza como accepted), y `ENG-060` (peripheral registry + hardware ports — migración `0014_site_peripherals.sql` con partial unique idx `(tenant_id, site_id, kind) WHERE is_active = 1`, 4 contracts ReceiptPrinter/CashDrawer/BarcodeScanner/PaymentTerminal, registry Strategy/Factory mirror del fiscal pattern con `getPeripheralAdapter` retornando null cuando no hay registration, 2 default drivers `system` + `manual` que NO tocan el legacy `webContents.print()` IPC path, admin tRPC router con 6 procedures, web admin UI `/peripherals` con site selector + lista por kind + register/test/edit/toggle/remove + driver picker filtrado con hints `Disponible con ENG-062/063`; `hardware_outbox` deferred a ENG-062 — ADR-0003 amended). Next: hardware readiness (ENG-061..062), sync/operations/resilience (ENG-064..067), y expansion architecture (ENG-068..070). |
