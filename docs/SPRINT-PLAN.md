...
1. **`ENG-025` (Shipped)** — critical security closure landed first and unconditionally. Closed the SEC-1 HIGH finding (Electron IPC bridge bypassing tenant scope) plus three MED findings (rate-limit, XSS, logout sessionVersion). Commits `5734d0a` + `6471007`.
2. **`ENG-026` (Shipped)** — Vite 7→8 + `@vitejs/plugin-react` 4→6 + `@types/node` 22→24 coordinated bump. No runtime regressions; bundle deltas ≤1% vs ENG-016 pass 4 baseline; `npm run build --workspace=@puntovivo/web` now ~470 ms via Rolldown.
3. **`ENG-029` — remaining Phase 0 hygiene tail** — hotspot splits triggered defensively, only when those files are next touched. `ENG-027` and `ENG-028` are already closed, so this is now the only open hardening-tail item before AI Wave 1.
4. **`ENG-030..ENG-033` — AI Wave 1** — `ENG-030`, `ENG-031`, and `ENG-032` are shipped. Last eligible Phase 1 ticket is `ENG-033` (semantic product search + auto-categorization), now unblocked because `ENG-044` activated the OpenAI provider. Ollama remains parked for `ENG-040`.
5. **`ENG-034..ENG-036` — Multi-country fiscal engine** — `ENG-034` FISCAL-CORE refactor blocks the country packs; `ENG-035` (MX) and `ENG-036` (CL) can run in parallel once the contract is in place. Argentina / Peru / Brazil packs queue after MX + CL are in sandbox-validated state.
6. **`ENG-037..ENG-038` — Sync + payment rails** — libSQL/Turso spike + LATAM payment rails (Wompi, Bold, ePayco, Mercado Pago, Nequi) with AI-assisted reconciliation.
7. **`ENG-039..ENG-040` — Vertical restaurant MX + AI Wave 2** — restaurant mode integrated with the MX pack; vision + voice features layer on the AI foundation.
8. **Gated tickets** stay parked until their gate clears: `ENG-022` hardware POS (test lab), `ENG-021` fiscal PT integration (signed contract + 5 more gates), `ENG-023` Bold datáfono (depends on `ENG-022` adapter interface + Bold sandbox).
9. **`ENG-024`** (inter-site transfer reservation) is deferred unless a multi-site tenant pushes for it.
