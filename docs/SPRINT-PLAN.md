...
| Foundation Reset | [`ENG-050..ENG-070`](./ROADMAP.md) | Partial (7 / 21 closed) | New sellability + foundation wave. **Closed so far**: `ENG-050` (roadmap truth sync + `SELLABILITY.md` go/no-go index), `ENG-051` (Foundation ADR pack), `ENG-052` cerrado completo en dos slices — `ENG-052a` foundation (schema 0010/0011, `devices` + `idempotency_keys` + reserva atómica, middleware `commandEnvelope`, `auth.changePassword` como proof) y `ENG-052b` close-out (17 procedures críticas + 9 test files adaptados + `useCriticalMutation` generalizado + Electron preload + Fastify request-logger hook), `ENG-053` (operation journal + outbox kernel — migración 0012 con `operation_events` + `operation_effects` + `operation_errors` + `outbox_metadata`; servicio `services/operation-journal/` y kernel `lib/outbox/` con factory `createOutboxKernel<TStatus,TPayload>` + worker base `tickOutbox` + retry policy `BOUNDED_EXPONENTIAL_BACKOFF` + atomic claim via `claim_token`; pattern docs nuevos en `architecture/patterns/operation-journal.md` y `architecture/patterns/outbox-kernel.md`), `ENG-054` (extract `completeSale` application service — nuevo directorio `packages/server/src/application/` con use-case discriminado `{ mode: 'fresh' \| 'fromDraft' }`; policies puras, `getSaleRecord`, y `safelyEmitFiscalDocument` reubicados; `sales.create` y `sales.completeDraft` thin wrappers; primer service-level consumer del journal — `operation_effects` se pueblan automáticamente en cada venta), `ENG-055` (extract sale lifecycle services — tres use-cases nuevos `application/sales/{returnSale,voidSale,discardDraft}.ts` que terminan la extracción de la mitad reversa del lifecycle. Policies compartidas extraídas: `inventory-policy.ts` con `reverseSaleItemsStock`, `fiscal-policy.ts` con `getOriginalDeeCufe`, y `policies.ts` extendido con `buildReturnedSaleNotes` + `buildVoidedSaleNotes` + `getPersistedCashContribution`. `assertCashSessionStillOpen` + `insertCashMovement` movidos a `services/cash-session.ts` con UNA sola fuente de verdad — completeSale.ts (ENG-054) actualizó imports. Router `sales.ts` simplificado de 1692 a ~990 líneas con dead code eliminado. Las tres operaciones ahora son service-level consumers del journal de ENG-053: returnSale emite `sale_row` + `sale_return_row` + N `inventory_movement` + `cash_movement` + 2 `sync_queue_emit` + `audit_log` + opcional `fiscal_emit`; voidSale análogo con cash_movement condicional al status del original session; discardDraft `sale_row` + N `inventory_movement` + `sync_queue_emit` + `audit_log` (NUNCA cash_movement ni fiscal_emit). 27 invariant tests nuevos sin tRPC + 58 tests existentes verde sin modificar. Server coverage 84.00/69.88/80.28/85.11. ZERO breaking changes en el contrato tRPC), y `ENG-056` (cash session aggregate boundary — nuevo directorio `packages/server/src/application/cash-sessions/` espejo de `application/sales/` con tres use-cases `openCashSession.ts` + `closeCashSession.ts` + `recordCashMovement.ts`; helpers compartidos `pending-checks.ts` + `journal-effects.ts` + `types.ts`. Las tres procedures críticas en `routers/cashSessions.ts` son ahora thin wrappers via `buildCashSessionContext`. Nueva query `cashSessions.pendingChecks` con counts + samples cap 5, gated mirror de `cashSessions.movements`. `insertCashMovement` widened a full `CashMovementType` enum eliminando el último INSERT directo en `cash_movements`. `closeCashSession` añade `assertCashSessionStillOpen` TOCTOU + integra pending checks en audit log metadata + emite `pending_warning` journal effects post-commit por categoría no-cero; close NUNCA bloquea. `openCashSession` añade el `cash_session.open` audit log que faltaba (paridad con close). 35 invariant tests + 3 integration cases nuevos. Server 963 tests verde, coverage 84.15/69.98/80.5/85.25). Next: fiscal readiness (ENG-057..ENG-058 consume el `fiscal_outbox` definido en ADR-0003 vía el kernel ENG-053), hardware readiness, sync/operations/resilience, y expansion architecture. |
