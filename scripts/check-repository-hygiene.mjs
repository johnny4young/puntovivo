...
/**
 * Paths whose content is generated by a tool from immutable published git
 * history rather than authored here.
 *
 * CHANGELOG.md is owned by release-please, which renders it from commit
 * subjects. Those subjects are already public in the repository's own log and
 * PR titles and cannot be rewritten, so scanning the rendered file protects
 * nothing that history has not already published. Hand-sanitizing it is worse
 * than pointless: editing the file out from under release-please broke its
 * parsing anchor, and the next release regenerated 169 lines of historical
 * entries -- ticket ids included -- turning main red on the release commit.
 *
 * Only the internal-ticket rule is relaxed. Private-path and dead-link checks
 * still apply to these files.
 */
const GENERATED_CONTENT_PATTERNS = [/^CHANGELOG\.md$/];
...
export function inspectRepositoryFile(path, content) {
  ...
  const isGenerated = GENERATED_CONTENT_PATTERNS.some(pattern => pattern.test(path));
  if (!isGenerated && INTERNAL_TICKET_PATTERN.test(content)) {
