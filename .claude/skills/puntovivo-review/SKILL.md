...
description: External pre-commit reviewer for the Puntovivo POS monorepo. Use this skill any time the operator wants a second pair of eyes on the staged diff BEFORE the commit lands — phrases like "revisa el staged", "haz review antes de commitear", "review externo del staging", "review pre-commit", "valida lo staged antes de subir", "audit the staged diff", "review my changes", or any context where the operator has staged work and wants verification. Also triggers on explicit `/puntovivo-review`. The skill operates as an INDEPENDENT reviewer, not a second implementer — the implementer has business + design context the reviewer does not, so design changes / ambiguities / security concerns go to the report, NOT to the diff. Real bugs (typos, broken imports, stale paths, missing i18n keys, multi-tenant leaks, accessibility gaps, type debt, broken tests) get fixed inline as unstaged edits so the operator can see them in `git diff` separated from the implementer's `git diff --cached`. The skill runs gates (`ci:server`, `ci:web`, `locale-parity`), invokes review skills (`typescript-react-reviewer`, `node`), checks repo-specific invariants (multi-tenant scope, i18n parity, sandbox, native deps, doc sync), and emits a canonical Review Guide with verdict (READY TO COMMIT / NEEDS DESIGN DISCUSSION / BLOCKED). Always prefer this skill when the operator asks "is the staged work correct?" or "review what I have staged" — never let the operator commit unreviewed work just because the question was casual. Skip only when there is genuinely nothing staged or when the operator is mid-Phase-2 of `puntovivo-ship` (the executing-implementer role); resume reviewing once the staging is final.
---
...
| `docs/README.md` | Authority index between the planning files. |
